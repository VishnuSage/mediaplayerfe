...
function Category() {
  ...
  const videoDrop = async (e, id) => {
    ...
    console.log(
      `Video with id ${videoid} need to put inside category with id ${id}`
    );
  ...
  const dragOver = (e) => {
    // ondragOver method will trigger page refresh, so the videoID we are passing may get lost
    e.preventDefault();
  ...
  const handleDeleteVideo = async (videoId) => {
    ...
    if (response.status === 200) {
      // Update the category videos
      const updatedCategory = allCategory.find((category) => category.id === item.id);
          ...
          >
            ...
            {item.allVideos.length > 0 ? (
              item.allVideos.map((card, index) => (
                <VideoCard key={index} displayVideo={card} onDelete={() => handleDeleteVideo(video.id)} />
              ))
            ) : (
